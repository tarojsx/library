...
import type { Chart, Global, ChartParams, Data, DataRecord } from '@antv/f2'
import { Canvas, ITouchEvent } from '@tarojs/components'

import { uuid } from '../utils'

interface F2CoreProps<TRecord extends DataRecord = DataRecord> {
    className?: string
    style?: React.CSSProperties
    config: { Chart: typeof Chart; Global: typeof Global } & ChartParams
    data: Data<TRecord>
    fontFamily?: string
    children(renderProps: { chart: Chart<TRecord>; data: Data<TRecord> }): any
}

export declare namespace F2Core {
    type Props<TRecord> = F2CoreProps<TRecord>
}

export const F2Core = function <TRecord extends DataRecord = DataRecord>(props: F2CoreProps<TRecord>) {
    const {
        className,
        style = {},
        config: { Chart, Global, ...config },
        data,
        fontFamily,
        children,
    } = props
